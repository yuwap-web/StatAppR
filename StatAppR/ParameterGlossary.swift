...
struct ParameterGlossary {
    ...
    private static let glossary: [String: String] = [
        ...
        "variables": "分析に使用する複数の変数。記述統計など複数列を一度に分析する場合。",
        "person_id": "個人を識別するID。複数行が同一人物のデータを示す。",
        "unit": "分析単位。個人、組織、地域など分析の最小単位。",
        "treat": "処置・治療を受けたかどうかを示す二値変数。0/1で表現。",
        "treatment_group": "処置群を示すグループ変数。対照群と比較する対象グループ。",
        "treat_unit": "処置の単位。個人レベル、クラスターレベルなど。",
        "treat_time": "処置が実施された時刻または時期。介入のタイミング。",
        "w": "重み。観測値の重みづけに使用される一般的な変数名。",
        "stop": "観測・治療の終了時刻。追跡期間の終点。",
        "event_time": "イベント発生時刻。追跡開始からイベント発生までの経過時間。",

        // ===== Advanced/Technical Parameters =====
        "a": "一般的な説明変数・共変量。分析に使用される変数。",
        "trim": "トリミング。極端な値を除外する割合。傾向スコアマッチングで使用。",
        "conf_int": "信頼区間レベル。デフォルト0.95は95%信頼区間を示す。",
        "robust_se": "ロバスト標準誤差の計算方法。HC0、HC1など複数の選択肢。",
        "scale": "スケーリング方法。標準化（scale）など前処理パラメータ。",
        "mode": "分析モード。複数の手法から選択する場合に使用。",
        "stabilized": "安定化の有無。安定化重み使用（TRUE）or非使用（FALSE）。",
        "ps_model": "傾向スコアモデルの種類。ロジスティック回帰など。",
        "smd_threshold": "標準化平均差（SMD）の閾値。通常0.1以下で均衡。",
        "check_ph": "比例ハザード仮定の検定有無。Cox回帰の前提条件を確認。",
        "compute_vif": "VIF（分散膨張係数）の計算有無。多重共線性チェック。",
        "drop_never_treated": "対照群のみの対象者を除外するか。処置があった対象者のみを使用。",
        "cv_folds": "交差検証のフォルド数。モデル評価に使用。",
        "ncomp_max": "PCA最大成分数。主成分分析の次元削減。",
        "ref_k": "参照カテゴリ（リファレンス）。カテゴリ変数の基準レベル。",
        "max_lead": "リード期間の最大値。将来の効果を検証する先行期間。",
        "min_lag": "ラグ期間の最小値。過去の効果を検証する遅延期間。",
        "case_window": "ケースの定義期間。イベント前後の判定期間（日数）。",
        "control_window": "対照群の定義期間。比較対象を定義する期間（日数）。",
        "placebo_shift": "プラセボ効果の検証シフト量。時間経過による影響を測定。",
    ]
